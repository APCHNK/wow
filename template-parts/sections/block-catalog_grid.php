<?php
// Render context-aware catalog grid:
//   - on a category or post-type archive page:
//       * cards repeater filled -> iterate it, each card links to a category or a project
//       * cards repeater empty  -> auto fallback (children on parent, projects on leaf/CPT archive)
//   - elsewhere: nothing
if (!is_tax('project_category') && !is_post_type_archive('wedding_project')) return;
...
function wow_catalog_card($image_url, $thumb_html, $link, $title, $country, $desc_slides, $country_svg, $fallback_alt = '') {
    ...
    <div class="project">
        ...
        <div class="project-info">
            ...
            <?php endif; ?>
            <a href="<?php echo esc_url($link); ?>" class="project-link">Show more</a>
        </div>
    </div>
    <?php
}
...
<section class="wedding-projects-catalog">
    ...
    if (!empty($cards)) :
        foreach ($cards as $card) :
            ...
            $title_top = (string) ($card['title_top'] ?? '');
            ...
            if ($target === 'project') {
                ...
                $title = $title_top !== '' ? $title_top : get_the_title($pid);
                $thumb = $image ? '' : get_the_post_thumbnail($pid, 'large');
                wow_catalog_card($image, $thumb, $link, $title, $country, $desc, $country_svg, get_the_title($pid));
            } else {
                ...
                $title = $title_top !== '' ? $title_top : $child->name;
                wow_catalog_card($image, '', $link, $title, $country, $desc, $country_svg, $child->name);
            }
        endforeach;
    elseif ($has_children) :
        foreach ($children as $child) :
            wow_catalog_card('', '', get_term_link($child), $child->name, '', [], $country_svg, $child->name);
        endforeach;
    else :
        // Leaf category OR /wedding-projects/ post-type archive.
        if (have_posts()) : while (have_posts()) : the_post();
            $thumb = get_the_post_thumbnail(get_the_ID(), 'large');
            wow_catalog_card('', $thumb, get_permalink(), get_the_title(), '', [], $country_svg, get_the_title());
        endwhile; endif;
    endif;
    ?>
    </div>
